...
class Car {
    private: 
        ...
        char model[25];
    public:
        void vivesti_mashiny(Car brand[25], Car model[25]) {
            std::cout << "Бренд: " << brand << " Модель: " << model << std::endl;
        };
        void vvod_zhachenii(char brend[], char model[]) {
            Car::brand[25] = brend[25];
            Car::model[25] = model[25];
            vivesti_mashiny();
        }
};
...
int main()
{
    ...
    mashina.vivesti_mashiny(model, brend);
